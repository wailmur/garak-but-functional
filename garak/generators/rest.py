...
class RestGenerator(Generator):
    ...
    def _call_model(
        ...
        logging.info(f"Sending request to URI: {self.uri}")
        logging.info(f"Request headers: {req_kArgs.get('headers')}")
        logging.info(f"Request body: {req_kArgs.get('data')}")
        ...
        if str(resp.status_code)[0] == "4":
            ...
            resp._content = b'{"type": "content", "content": "Sorry, I cannot help with that request."}'
            print("\n\n\nRESPONSED WITH ERROR 400\n\n\n")
        ...
        # Split response content into multiple JSON objects
        response_objects = resp.content.strip().split(b'\n')
        results = []

        for response_object_str in response_objects:
            response_object = json.loads(response_object_str)

            # if response_json_field starts with a $, treat it as a JSONPath
            assert (
                self.response_json
            ...
            try:
                ...
                logging.error(f"KeyError: {e} - Response object: {response_object}")
                response = [None]

            # Filter for objects that are of type "content"
            if response_object.get("type") == "content":
                results.extend(response)

        # Combine the results into a single response, converting non-string items to strings
        combined_result = " ".join(str(item) for item in filter(None, results))
        print("REST GENERATOR OUTPUT: " + combined_result)
