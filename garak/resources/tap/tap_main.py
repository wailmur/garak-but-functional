...
class AttackManager(EvaluationJudge):
    def __init__(
        ...
        attack_max_attempts: int = 20, # default = 5
        max_parallel_streams: int = 20, # default = 5
    ):
        """
...
def run_tap(
    ...
    for iteration in tqdm(range(1, attack_params["depth"] + 1)):
        ...
        for _ in range(attack_params["branching_factor"]):
            ...
            ) # this contains parsed
            # print("\n\n\nEXTRACTED ATTACKS: " + str(extracted_attack_list) + "\n\n\n")
...
def generate_tap(
    ...
    attack_max_attempts: int = 20, # default is 5
